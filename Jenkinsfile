    ...
    stages {
        ...
        stage('Build & SonarScanner'){
		     steps{
                ...
                echo 'Build'
               // SonarAnalysis()				
                bat "\"${tool '.net 4'}\\msbuild.exe\" mvcToDoList/mvcToDoList.sln /p:Configuration=Debug /p:Platform=\"Any CPU\" /p:VisualStudioVersion=15.0 /p:ProductVersion=1.0.0.${env.BUILD_NUMBER} /v:diag"
                bat "\"${tool '.net 4'}\\msbuild.exe\" /t:Package mvcToDoList/mvcToDoList/mvcToDoList.csproj"
